...
int main(int argc, char* argv[]){
    ...
    char* demo1[17] = {const_cast<char *>("resourceupdate"), const_cast<char *>("--ri"), const_cast<char *>("resource_0001"),
    ...
    commandline.cmd_desc_func_map[cmdname] =  [](shared_ptr<po::options_description> res_cmdline_options)->void {
        ...
        command.add_options()
        ("ri", po::value<string>(), "存储资源UUID")
        ("rn", po::value<string>(), "存储资源名称")
        ("ci", po::value<string>(), "超算中心UUID")
        ("cn", po::value<string>(), "超算中心名称")
        ("tc", po::value<int64_t>(), "存储资源空间容量大小")
        ("ac", po::value<int64_t>(), "已分配存储资源空间容量大小")
        ("mgs",po::value<string>(), "资源mgs地址")
    ...
    commandline.cmd_do_func_map[cmdname] =  [&](shared_ptr<po::variables_map> res_variables_map)->void {
        if (res_variables_map->count("ri"))storage_src_id = (*res_variables_map)["ri"].as<string>();
        if (res_variables_map->count("rn"))storage_src_name = (*res_variables_map)["rn"].as<string>();
    ...
    try{
        ...
        ipcClient.set_callback_func([&](IPCMessage msg)->void {
            ...
            if(ipcresult == "")cout << "resource register fail" << endl;
