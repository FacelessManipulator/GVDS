...
int main(int argc, char* argv[]){
    ...
    // 1、用户登录
    char* demo1[9] = {const_cast<char *>("adcam"),
...
//    CmdLineProxy commandline(2, demo2);
    ...
    commandline.cmd_desc_func_map[cmdname] =  [](std::shared_ptr<po::options_description> sp_cmdline_options)->void {
        po::options_description command("管理员给[指定用户]添加[指定位置]的账户映射");
        command.add_options()
    ...
    try{
        ...
        ipcClient.set_callback_func([&](IPCMessage msg)->void {
            ...
            if (ipcresult == "0"){
                std::cout << "建立账户映射成功" << std::endl;
            }
            else if(ipcresult == "1"){
                std::cout << "建立账户映射失败" << std::endl;
            }
            else if(ipcresult == "33"){
                std::cout << "权限不足：无法建立账户映射" << std::endl;
            }
            else{
