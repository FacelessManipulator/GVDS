 ...
 * 
 * 日志等级的设计是用来过滤日志。例如同样的日志打印代码`dout(20) << "DEBUG DATA" << dendl;`，在日志收集等级小于20时，
 * 并不会输出到日志文件中。又例如`dout(-1) << "SYSTEM ERROR" << dendl;`，通常是系统无法忽略的致命错误，将会被打印到stderr中。
 * 
 * 日志类的使用方式比较简单，在`#include "debug.h"`以后，直接使用宏`dout(${DEBUG_LEVEL}) << "你的DEBUG信息" << dendl;`即可，也
 * 可以多行使用。
 * ``` cpp
...
class Log : private Thread {
  using log_lock = std::chrono::system_clock; ///< 使用标准库的系统时钟作为时钟，可能会因为系统时间调整而造成日志时间错乱
  log_lock clock;                             ///< 只在创建日志记录entry的时候用到，用来打时间戳timepoint

  pthread_mutex_t m_queue_mutex;  ///< 日志提交队列m_new的锁
  pthread_mutex_t m_flush_mutex;  ///< 日志flush的锁，用来互斥操作文件和冲刷日志的操作
  pthread_cond_t m_cond_loggers;  ///< 当日志队列满的时候用来让其他线程等待的信号量，可能会阻塞调用dout的线程
  pthread_cond_t m_cond_flusher;  ///< 当日志队列空的时候用来让日志线程等待的信号量

  pthread_t m_queue_mutex_holder; ///< 用来记录队列锁的持有者，用来让其他线程判断自己持有log锁
  pthread_t m_flush_mutex_holder; ///< 用来记录冲刷锁的持有者，用来让其他线程判断自己持有log锁
 ...
 public:
  ...
  ~Log() override;  ///< 析构函数，在析构前必须首先调用stop()，完成冲刷缓冲区、终止线程等操作
  ...
  void set_log_level(int log);    ///< 设置普通的日志收集等级，默认为10，这意味着日志等级小于等于10的都会被收集到日志文件中
  ...
  void submit_entry(EntryPtr ep);   ///< 提交日志条目，该函数通常被其他线程才用dendl宏的形式调用
