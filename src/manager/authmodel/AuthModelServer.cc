...
void AuthModelServer::self_AuthmemberaddRest(const Rest::Request& request, Http::ResponseWriter response){
    ...
    dout(-1) << "====== end AuthModelServer function: self_AuthmemberaddRest ======"<< dendl;
}
int AuthModelServer::self_Authmemberadd(SelfAuthSpaceInfo &auth_space){
    ...
    if (value.compare("fail") == 0){
        dout(-1) << "SpacePermissionSyne fail" << dendl;
        return -1;
    }
...
void AuthModelServer::self_AuthmemberdelRest(const Rest::Request& request, Http::ResponseWriter response){
    ...
    dout(-1) << "====== end AuthModelServer function: self_AuthmemberdelRest ======"<< dendl;
}
int AuthModelServer::self_Authmemberdel(SelfAuthSpaceInfo &auth_space){
    ...
    for(m_iter = auth_space.memberID.begin(); m_iter != auth_space.memberID.end(); m_iter++){
        ...
        string m_value = p_usermodel->getLocalAccountinfo(*m_iter, hostCenterName); 
        if (m_value.compare("fail") == 0){
            dout(-1) << "member delete fail" << dendl;
            return -1;
        }
        LocalAccountPair member_localpair;
        member_localpair.deserialize(m_value);  //member_localpair.localaccount 这个是账户名
...
void AuthModelServer::self_AuthgroupmodifyRest(const Rest::Request& request, Http::ResponseWriter response){
    ...
    dout(-1) << "====== end AuthModelServer function: self_AuthgroupmodifyRest ======"<< dendl;
...
int AuthModelServer::self_Authgroupmodify(AuthModifygroupinfo &groupinfo){
    ...
    // if (value.compare("fail") == 0){
    //     dout(-1) << "self_Authgroupmodify fail" << dendl;
    //     return -1;
    // }
    // LocalAccountPair localpair;
    // localpair.deserialize(value);  //localpair.localaccount 这个是账户名
    ...
    string localstoragepath = *(HvsContext::get_context()->_config->get<std::string>("manager.data_path"));
        dout(-1) <<"localstoragepath: " << localstoragepath << dendl;
    string spacepath = localstoragepath + "/" + spacemeta.spacePath; //这个可能不是最终的路径，需确认
        dout(-1) << "final path :" << spacepath << dendl;
...
void AuthModelServer::self_AuthSPDRest(const Rest::Request& request, Http::ResponseWriter response){
    dout(-1) << "=========enter：self_AuthSPDRest============" << dendl;
    auto info = request.body();
    dout(-1) << info << dendl;
    ...
    dout(-1) << "=========end：self_AuthSPDRest============"<< dendl;
}

int AuthModelServer::self_AuthSPD(SelfSPD &mySPD){
    
    dout(-1) << "=========enter：self_AuthSPD============" << dendl;
    Space spacemeta;
    ...
    dout(-1) << "group_cmd: " << group_cmd << dendl;
    system(group_cmd.c_str());

    dout(-1) << "=========end：self_AuthSPD============" << dendl;
    return 0;
...
int AuthModelServer::ZonePermissionAdd(std::string zoneID, std::string ownerID){
    ...
    int f1_flag = f3_dbPtr->set(auth_prefix + zoneID, value);
    if (f1_flag != 0){
        dout(-1) << "fail" << dendl;
        return -1;
    }
...
int AuthModelServer::SpacePermissionSyne(std::string spaceID, std::string zoneID, std::string ownerID, std::vector<std::string> memberID){
    dout(-1) << "=======start: SpacePermissionSyne=======" << dendl;
    ...
    auto [pvalue, error] = f3_dbPtr->get(auth_prefix + zoneID);
    if(error){
        dout(-1) << "fail" << dendl;
        return -1;
    }
    ...
    if (zone_error){
        dout(-1) << "authmodelserver: zone search fail" << dendl;
        return -1;
    }
    dout(-1) << "112.2" << dendl;
    Zone zone_content;
    ...
    for (auto& space_iter : result) {
        ...
        //测试时假设是Shanghai
        string hostCenterName = spacemeta.hostCenterName;
        dout(-1) <<"spacemeta.hostCenterName: " << spacemeta.hostCenterName << dendl;
        //string hostCenterName ="Shanghai";
        dout(-1) <<"test hostCenterName: " << hostCenterName << dendl;
...
int AuthModelServer::ZoneMemberAdd(string zoneID, string ownerID, vector<string> memberID){
    ...
    if (c_error){
        dout(-1) << "authmodelserver: get center_information fail" << dendl;
        return -1;
    }
    CenterInfo mycenter;
    ...
    for (auto space_iter : result){
        ...
        string hostCenterName = spacemeta.hostCenterName;
        dout(-1) << "hostCenterName: " << hostCenterName << dendl;
        ...
        { 
            ...
            response.then(
            [&](Http::Response res) {
                dout(-1) << "Response code = " << res.code() << dendl;
                auto body = res.body();
                if (!body.empty()){
                    dout(-1) << "Response body = " << body << dendl;
                    //your code write here
                    if (body != "0"){
                        dout(-1) << "body != 0, fail " << dendl;
                        return_flag = -1;
                    }
...
int AuthModelServer::ZonePermissionDeduct(string zoneID, string OwnerID){
    ...
    int flag = f3_dbPtr->remove(auth_prefix + zoneID);
    if(flag){
        dout(-1) << "remove auth fail" <<dendl;
        return -1;
    }
    else{
        dout(-1) << "remove auth success" << dendl;
        return 0;
    }
}
    ...
    for (auto space_iter : result){ //for其实没有用，因为vector只有一个spaceID，即每次只删除一个spaceID
        ...
        if(ManagerID == spacemeta.hostCenterID){ //本地
            if(self_AuthSPD(mySPD) != 0){
                dout(-1)<< "删除空间权限失败" << dendl;
                return_flag = -1;
            }
            else{
                dout(-1)<< "删除空间权限成功" << dendl;
            }
        }
        else{ //远端
            dout(-1) <<"发送到远端" << dendl;
...
void AuthModelServer::AuthModifyRest(const Rest::Request& request, Http::ResponseWriter response){
    dout(-1) << "====== start AuthModelServer function: AuthModifyRest ======"<< dendl;
    ...
    dout(-1) << "====== end AuthModelServer function: AuthModifyRest ======"<< dendl;

}

int AuthModelServer::AuthModify(string hvsID, string zonename, string modify_groupauth){
    dout(-1) << "enter AuthModify"<< dendl;
...
//////
    ...
    cout << "aaaaaa: " << string(query) << endl;
    auto [vp, err] = zonePtr->n1ql(string(query));
    if(vp->size() == 0) {
        dout(-1) << "find zoneID fail" << dendl;
        return -1;
    }
    ...
    tmp.deserialize(n1ql_result);
        
    std::string zoneID = tmp.zoneID;
    dout(-1) << "****zoneID: " << zoneID << dendl;
...
///////////////////////
    ...
    auto [pzone_value, zone_error] = zone_dbPtr->get(zone_prefix + zoneID);
    if (zone_error){
        dout(-1) << "authmodelserver: auth modify fail" << dendl;
        return -1;
    }
    ...
    {   
        ...
        int f3_f = f3_dbPtr->set(auth_prefix + zoneID, m_value);
        if (f3_f != 0){
            dout(-1) << "fail" << dendl;
            return_flag = -1;
        }
...
void AuthModelServer::AuthSearchModelRest(const Rest::Request& request, Http::ResponseWriter response){
    dout(-1) << "====== start AuthModelServer function: AuthSearchModelRest ======"<< dendl;
    // bool valid = auth_token(request);
    // if (!valid){
    //     dout(-1) << "aaa here?" << dendl;
    //     response.send(Http::Code::Unauthorized, "33");
    //     return;
    // }

    auto info = request.body();
    dout(-1) << "info: " <<info << dendl;  //hvsid
    ...
    dout(-1) << "====== end AuthModelServer function: AuthSearchModelRest ======"<< dendl;
}


string AuthModelServer::AuthSearchModel(string &hvsID){
     dout(-1) << "enter AuthSearchModel"<< dendl;
    //1、查询hvsid对应的所有区域ID
    vector<Zone> result_z;
    ZoneServer *p_zone = static_cast<ZoneServer*>(mgr->get_module("zone").get());
        dout(-1) << "start: p_zone->GetZoneInfo" << dendl;
    bool result_b = p_zone->GetZoneInfo(result_z, hvsID);//sy函数
        dout(-1) << "end: p_zone->GetZoneInfo" << dendl;
    //string result;
    if( !result_b ){
        dout(-1) << "get zoneID info fail" <<dendl;
        return "fail";
    }
    ...
    auto [own, oerr] = accountPtr->get(user_prefix + hvsID);
    if (!oerr)
    {
          Account owner;
...
int AuthModelServer::subAuthSearchModel(Zone &myzone, string hvsID, string &r, string &w, string &x , string &identity,
    ...
    if(myzone.ownerID.compare(hvsID)==0){   // myzone.ownerID 这块比较特殊 存的其实是 name 因此这块就是比较name
        identity = "1";
    }
    else{
    ...
    auto [pvalue, error] = f3_dbPtr->get(auth_prefix + myzone.zoneID);
    if(error){
        dout(-1) << "fail" << dendl;
        return -1;
    }
