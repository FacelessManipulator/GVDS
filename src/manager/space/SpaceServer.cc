...
namespace hvs{
    ...
    void SpaceServer::router(Router& router) {
       ...
       Routes::Post(router, "/space/spacenumber", Routes::bind(&SpaceServer::SpaceNumberRest, this));
    }
...
//        std::string tmp_value = n1ql_result.substr(sizeof("{\"\":")+storagebucket.size()-1, n1ql_result.length()-(sizeof("{\"\":")+storagebucket.size())); // 处理返回的字符串问题
        ...
        return {StorageResource::prefix()+storage.storage_src_id, storage.host_center_id};
    }
    ...
    std::string SpaceServer::SpaceCheck(std::string ownerID, std::vector<std::string> memberID, std::string spacePathInfo)
    {
        ...
        "and META().id like '%s%%'", 
        spacebucket.c_str(), tmpm.hostCenterID.c_str(), tmpm.storageSrcID.c_str(), tmpm.spacePath.c_str(), space_prefix.c_str());
...
//            space.spacePath.replace(pos, oldSpaceName.length(), newSpaceName);
    ...
    int SpaceServer::SpaceSizeChange(std::string spaceID, int64_t newSpaceSize) {
        ...
        Space spacejson;
        std::shared_ptr<hvs::Datastore> spaceptr =hvs::DatastoreFactory::create_datastore(spacebucket, hvs::DatastoreType::couchbase, true);
        auto [vp, err] = spaceptr->get(space_prefix + spaceID); // 通过spaceID获取到描述空间的json数据；
        if(err)
        {
            return EAGAIN;
        }
    ...
    void SpaceServer::SpaceNumberRest(const Rest::Request& request, Http::ResponseWriter response){
        dout(10) << "====== start ZoneServer function: SpaceNumberRest ======"<< dendl;
        auto info = request.body();
    ...
    int SpaceServer::SpaceNumber(std::string hostCenterName)
    {
        ...
        char query[256];
        snprintf(query, 256, 
        "select UUID, name, capcity, SC_UUID, Storage_UUID, root_location, hostCenterName, storageSrcName, Status from `%s` where hostCenterName = \"%s\" "
        "and META().id like '%s%%'", 
