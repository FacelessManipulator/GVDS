...
void SpaceServer::router(Router& router) {
  Routes::Post(router, "/space/rename",
               Routes::bind(&SpaceServer::SpaceRenameRest, this));
  Routes::Post(router, "/space/changesizeapply",
               Routes::bind(&SpaceServer::SpaceSizeChangeApplyRest, this));
  Routes::Post(router, "/space/changesize",
               Routes::bind(&SpaceServer::SpaceSizeChangeRest, this));
  Routes::Post(router, "/space/spaceusage",
               Routes::bind(&SpaceServer::SpaceUsageRest, this));
  Routes::Post(router, "/space/spaceusagecheck",
               Routes::bind(&SpaceServer::SpaceUsageCheckRest, this));
  Routes::Post(router, "/space/spacenumber",
               Routes::bind(&SpaceServer::SpaceNumberRest, this));
  Routes::Post(router, "/space/replica",
               Routes::bind(&SpaceServer::SpaceReplicaRest, this));
}
...
void SpaceServer::GetSpaceInfo(std::vector<Space>& result_s,
                               std::vector<std::string> spaceID) {
  Space tmps;
...
std::string SpaceServer::SpaceCreate(std::string spaceName, std::string ownerID,
  ...
  // TODO: 空间位置选择接口
  auto [storid, hostid] =
      GetSpaceCreatePath(spaceSize, tmpm.hostCenterName, tmpm.storageSrcName);
  ...
  int addret = SpaceSizeAdd(tmpm.storageSrcID,
                            spaceSize);  // 把新修改的空间用量记录到聚合模块中

  if (addret != 0) {
    return "-1";  // 聚合模块修改失败，返回false字符串，用于之后的判断；
  }
  // 2、获取账户映射信息，创建空间目录,返回spacePath
  // TODO： 把空间组和用户使用 chown 修改； owner; ownerID, hostCenterID bool
  // (&string &string, owner, hostcenterid) || string (owner, hostcenterid);
  ...
  new_space.spacePath =
      tmp_uuid;  // TODO: 默认当前 spacepath
                 // 在存储集群顶层，且空间的名字为相对lustre挂载点的路径，并且使用UUID作为文件夹的名字
  // 3、权限增加模块 ： 不需要
  // 4、空间分配容量记录到聚合模块中
  ...
  if (new_space.spaceName == "" || new_space.spaceSize == 0) return "-5";
  std::string tmps_key = space_prefix + new_space.spaceID;
...
std::tuple<std::string, std::string> SpaceServer::GetSpaceCreatePath(
  ...
  if (storageSrcName.empty()) {
    snprintf(query, 256,
             "select "
             "assign_capacity,host_center_id,host_center_name,mgs_address,"
             "state,storage_src_id, storage_src_name, total_capacity from `%s` "
  ...
  if (vp->size() == 0) {
    dout(10) << "GetSpaceCreatePath: "
                "未查找到制定的存储ID，根据空间名和数据中心的名字！"
             << dendl;
    return {"", ""};
  }

  // 2019年08月11日 默认存储集群选择策略为：从查找到的所有
  // 存储集群中选择剩余空间最多的集群，进行获取其 id 和 host_center_id
  ...
  //        n1ql_result.substr(sizeof("{\"\":")+storagebucket.size()-1,
  //        n1ql_result.length()-(sizeof("{\"\":")+storagebucket.size())); //
  //        处理返回的字符串问题
  StorageResource storage;
  storage.deserialize(
      ite->second);  // 使用剩余存储空间最多的集群进行作为默认集群；
  ...
  return {StorageResource::prefix() + storage.storage_src_id,
          storage.host_center_id};
}
...
std::string SpaceServer::SpaceCheck(std::string ownerID,
  ...
  tmpm.deserialize(spacePathInfo);
  auto [storid, hostid] =
      GetSpaceCreatePath(0, tmpm.hostCenterName, tmpm.storageSrcName);
  if (storid.empty() && hostid.empty()) {
  ...
  char query[256];
  snprintf(query, 256,
           "select UUID, name, capcity, SC_UUID, Storage_UUID, root_location, "
           "hostCenterName, storageSrcName, Status from `%s` where SC_UUID = "
           "\"%s\" and Storage_UUID = \"%s\" and root_location = \"%s\" "
           "and META().id like '%s%%'",
           spacebucket.c_str(), tmpm.hostCenterID.c_str(),
           tmpm.storageSrcID.c_str(), tmpm.spacePath.c_str(),
           space_prefix.c_str());
...
int SpaceServer::SpaceDelete(std::vector<std::string> spaceID) {
  ...
  for (std::vector<std::string>::iterator m = spaceID.begin();
    ...
    // TODO:修改空间分配容量; 当前直接在聚合表中把空间容量删除；
    // 未检测实际使用的容量
...
void SpaceServer::SpaceRenameRest(const Rest::Request& request,
                                  Http::ResponseWriter response) {
  dout(10) << "====== start SpaceServer function: SpaceRenameRest ======"
           << dendl;
  ...
  dout(10) << "====== end SpaceServer function: SpaceRenameRest ======"
           << dendl;
}
...
int SpaceServer::SpaceRename(std::string spaceID, std::string newSpaceName) {
  ...
  if (!space.status)
    ...
    //            space.spacePath.replace(pos, oldSpaceName.length(),
    //            newSpaceName);
    int flag = spaceptr->set(space_prefix + spaceID, space.serialize());
    if (flag != 0)
      return -1;
    else
      return 0;
...
// YaoXu: 空间扩容缩容
void SpaceServer::SpaceSizeChangeRest(const Rest::Request& request,
                                      Http::ResponseWriter response) {
  auto info = request.body();
...
int SpaceServer::SpaceSizeChange(std::string spaceID, int64_t newSpaceSize) {
  ...
  if (ManagerID == spacejson.hostCenterID) {  //本地
    ...
    snprintf(url, 256, "http://%s:%s/space/spaceusage", tmp_ip.c_str(),
             tmp_port.c_str());
  ...
  //不需要记录数据库，查询容量完毕
  //-------------------
  int64_t realsizetoGB = (realsize / (1000 * 1000) + 1) /
                         1;  // 不够1GB向上取整，获取最终值，容量分配的粒度为GB
  ...
  if (newSpaceSize > spacejson.spaceSize) {
    int ret = SpaceSizeAdd(spacejson.storageSrcID,
                           newSpaceSize - spacejson.spaceSize);
    if (ret == 0) {
      spacejson.spaceSize = newSpaceSize;
      int flag = spaceptr->set(space_prefix + spaceID, spacejson.serialize());
      if (flag != 0) return EAGAIN;
    }
    return ret;
  } else if (newSpaceSize < spacejson.spaceSize) {
    int ret = SpaceSizeDeduct(spacejson.storageSrcID,
                              spacejson.spaceSize - newSpaceSize);
    if (ret == 0) {
      spacejson.spaceSize = newSpaceSize;
      int flag2 = spaceptr->set(space_prefix + spaceID, spacejson.serialize());
...
int SpaceServer::SpaceSizeAdd(std::string StorageID, int64_t add_size) {
  ...
  if (storage.assign_capacity + add_size > storage.total_capacity) {
    ...
    int flag = storptr->set(
        StorageID, storage.serialize());  // 容量改变，并重新存储到数据库中；
    if (flag != 0) return EAGAIN;
  }
  return 0;
}
...
// 注意此接口查找的 文件的大小 返回的单位 以 KB 为单位返回，121000表示 121000KB
// 121MB 注意此处 KB 与 KiB的区别；
void SpaceServer::SpaceUsageCheckRest(const Rest::Request& request,
                                      Http::ResponseWriter response) {
  dout(10) << "====== start SpaceServer function: SpaceUsageCheckRest ======"
           << dendl;
  auto info = request.body();
  ...
  response.send(Http::Code::Ok, result);  // point
  dout(10) << "====== end SpaceServer function: SpaceUsageCheckRest ======"
           << dendl;
}

std::vector<int64_t> SpaceServer::SpaceUsageCheck(
    std::vector<std::string> spaceID) {
  std::vector<int64_t> res;
  // string localstoragepath =
  // *(HvsContext::get_context()->_config->get<std::string>("manager.data_path"));
  ...
  for (const auto& space_iter : result) {  //对每个空间进行查询
    ...
    string spacepath =
        spacemeta.spacePath;  // TODO：每个空间的local spacepath
                              // 各异，都不是一样的，具体路径在配置文件中配置；
...
void SpaceServer::SpaceUsageRest(const Rest::Request& request,
                                 Http::ResponseWriter response) {
  dout(10) << "====== start SpaceServer function: SpaceUsageRest ======"
           << dendl;
...
int64_t SpaceServer::SpaceUsage(
    std::string spacepath) {  // 检测出来的文件的大小以KB为单位返回；注意此接口中spacepath为相对本机的路径，比如
                              // /tmp 就是 本机的 /tmp目录；
  string localstoragepath =
      *(HvsContext::get_context()->_config->get<std::string>(
          "manager.data_path"));
  ...
  // dout(10) << "解析后容量大小(数字):"<< formal_size << size_str[unit]<<
  // dendl; 根据容量后单位G,M,K统一转化为KB为单位的容量大小
  int64_t size = 0;
  if (size_str[unit] == 'G') {
...
void SpaceServer::SpaceSizeChangeApplyRest(const Rest::Request& request,
                                           Http::ResponseWriter response) {
  dout(10)
      << "====== start ZoneServer function: SpaceSizeChangeApplyRest ======"
      << dendl;
  auto info = request.body();
  ...
  dout(10) << "====== end ZoneServer function: SpaceSizeChangeApplyRest ======"
           << dendl;
}
...
void SpaceServer::SpaceNumberRest(const Rest::Request& request,
                                  Http::ResponseWriter response) {
  dout(10) << "====== start ZoneServer function: SpaceNumberRest ======"
           << dendl;
  auto info = request.body();

  dout(10) << "info: " << info << dendl;

  int result = SpaceNumber(info);
  response.send(Http::Code::Ok, json_encode(result));
  dout(10) << "====== end ZoneServer function: SpaceNumberRest ======" << dendl;
}
...
int SpaceServer::SpaceNumber(std::string hostCenterName) {
  ...
  snprintf(query, 256,
           "select UUID, name, capcity, SC_UUID, Storage_UUID, root_location, "
           "hostCenterName, storageSrcName, Status from `%s` where "
           "hostCenterName = \"%s\" "
           "and META().id like '%s%%'",
           spacebucket.c_str(), hostCenterName.c_str(), space_prefix.c_str());
...
// mark replica space
void SpaceServer::SpaceReplicaRest(const Rest::Request& request,
                                   Http::ResponseWriter response) {
  std::map<std::string, std::string> replicaParams;
  json_decode(request.body(), replicaParams);
  // check parameters, replica from space A to space B
  if (replicaParams.count("replicated") == 0 ||
      replicaParams.count("single") == 0) {
    response.send(Http::Code::Bad_Request);
    return;
  }
  int result = SpaceReplica(replicaParams);
  response.send(Http::Code::Ok, json_encode(result));
}

int SpaceServer::SpaceReplica(
    std::map<std::string, std::string>& replicaParams) {
  auto dbc = DatastoreFactory::create_datastore(spacebucket, couchbase, true);
  auto dbr = dynamic_pointer_cast<CouchbaseDatastore>(dbc)->get_raw_client();
  auto [res, err] =
      dbc->get(space_prefix + replicaParams["replicated"], "replica_spaces");
  // TODO: we not check whether single space is replicated or not, it should be
  // checked before this function
  if (err) {
    return err;
  }
  vector<string> replicated_spaces;
  json_decode(*res, replicated_spaces);
  if (replicated_spaces.size() >= 5) {
    return -E2BIG;
  }
  for (auto& it : replicated_spaces) {
    // insert single space into all replicated spaces's broadcast group
    dbr->arr_insert_uniq(space_prefix + it, "replica_spaces",
                         json_encode(replicaParams["single"]));
    dbr->arr_insert_uniq(space_prefix + replicaParams["single"],
                         "replica_spaces", json_encode(it));
  }
  dbr->arr_insert_uniq(space_prefix + replicaParams["replicated"],
                       "replica_spaces", json_encode(replicaParams["single"]));
  dbr->arr_insert_uniq(space_prefix + replicaParams["single"], "replica_spaces",
