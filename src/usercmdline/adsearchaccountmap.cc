...
int main(int argc, char* argv[]){
    ...
    // 1、用户登录
    char* demo1[9] = {const_cast<char *>("adsearcham"),
                       const_cast<char *>("-u"), const_cast<char *>("lbq")};
...
//    CmdLineProxy commandline(2, demo2);
    ...
    commandline.cmd_desc_func_map[cmdname] =  [](std::shared_ptr<po::options_description> sp_cmdline_options)->void {
        po::options_description command("管理员查询[指定用户]账户映射信息");
        command.add_options()
    ...
    try{
        ...
        ipcClient.set_callback_func([&](IPCMessage msg)->void {
            ...
            if(ipcresult == "1"){
                std::cout << "查询账户映射失败" << std::endl;
            }
            else if(ipcresult == "33"){
                std::cout << "权限不足：无法查询账户映射" << std::endl;
            }
            else if(ipcresult =="-1"){
                std::cout << "未获得对应账户信息，请确认信息正确！" << std::endl;
            }
            else{
                SCAccount person;
                person.deserialize(ipcresult);
                std::cout << "============================" << std::endl;
                std::cout << "虚拟数据空间账户名： " << username << std::endl;
                std::cout << "============================" << std::endl;

                std::vector<std::string>::iterator iter;
                for(iter = person.centerName.begin(); iter != person.centerName.end(); iter++){
                    std::cout << "映射的账户名: " << person.localaccount[*iter] << std::endl;
                    std::cout << "映射位置： " << *iter << std::endl; 
                    // srd::cout << "映射的账户密码: " << person.localpassword[*iter] << std::endl;
                    std::cout << "============================" << std::endl;
                }
            }
